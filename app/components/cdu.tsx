...
export default function App() {
  // Sample data structure for the menu
  const menuTree = [
    { name: 'Menu 1', items: ['Home', 'Lamps', 'Track', 'System Configuration'] },
    { name: 'Menu 2', items: ['Item 2.1', 'Item 2.2', 'Item 2.3'] },
    // Add more menus and items as needed
  ];

  // State for main and submenu indexes
  const [mainIndex, setMainIndex] = useState(0);
  const [subIndex, setSubIndex] = useState(0);

  // Handler functions for button clicks
  const handleRightClick = () => {
    setMainIndex((prevIndex) => (prevIndex + 1) % menuTree.length);
    setSubIndex(0); // Reset submenu index when changing main menu
  };

  const handleLeftClick = () => {
    setMainIndex((prevIndex) => (prevIndex - 1 + menuTree.length) % menuTree.length);
    setSubIndex(0); // Reset submenu index when changing main menu
  };

  const handleUpClick = () => {
    setSubIndex((prevIndex) => (prevIndex - 1 + menuTree[mainIndex].items.length) % menuTree[mainIndex].items.length);
  };

  const handleDownClick = () => {
    setSubIndex((prevIndex) => (prevIndex + 1) % menuTree[mainIndex].items.length);
  };

  // Get the current menu and submenu items
        ...
        <div className="bg-gray-400 drop-shadow-xl p-4 rounded-2xl w-[300px] h-[350px] flex flex-col justify-between">
          <div className="bg-gray-700 p-2 rounded">
            <div className="bg-gray-600 w-full h-6 mb-2 rounded">
              <p className="text-lime-400 w-1/2 h-6 rounded">{currentMenuItem}</p>
            </div>
